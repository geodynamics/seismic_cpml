...
! attenuation constants
  double precision, dimension(N_SLS) :: tau_epsilon_nu1, tau_sigma_nu1
...
! memory variable and other arrays for attenuation
  ...
  double precision, dimension(NX,NY,N_SLS) :: deltaKappa_over_tau_sigma
  integer :: i_sls
...
! compute deltaKappa_over_tau_sigma, which is a term needed to compute the evolution of the viscoacoustic memory variables
  if (VISCOACOUSTIC_ATTENUATION) then
    do j = 1,NY
      do i = 1,NX
        do i_sls = 1,N_SLS
          deltaKappa_over_tau_sigma(i,j,i_sls) = kappa_relaxed(i,j) * (tau_epsilon_nu1(i_sls)/tau_sigma_nu1(i_sls) - 1.d0) &
                                                      / dble(N_SLS * tau_sigma_nu1(i_sls))
        enddo
      enddo
    enddo
  else
    deltaKappa_over_tau_sigma(:,:,:) = ZERO
  endif
...
!-------------------------------------------------------
! update memory variables for viscoacoustic attenuation
!-------------------------------------------------------

! use the Auxiliary Differential Equation form, which is second-order accurate if implemented following
...
! this average of the two terms comes from eq (14) of Robertsson, Blanch and Symes, Geophysics, vol. 59(9), pp 1444-1456 (1994)
          memory_variable_R_dot(i,j,i_sls) = (memory_variable_R_dot_old(i,j,i_sls) &
             + DELTAT * (value_dvx_dx + value_dvy_dy)*deltaKappa_over_tau_sigma(i,j,i_sls) &
             - DELTAT * 0.5d0*memory_variable_R_dot_old(i,j,i_sls) / tau_sigma_nu1(i_sls)) &
                   / (1.d0 + 0.5d0 * DELTAT / tau_sigma_nu1(i_sls))
        enddo
...
! this average of the two terms comes from eq (13) of Robertsson, Blanch and Symes, Geophysics, vol. 59(9), pp 1444-1456 (1994)
                   0.5d0 * sum((memory_variable_R_dot(i,j,:) + memory_variable_R_dot_old(i,j,:)))) * DELTAT
...
! define location of the source
  ...
  pressure(i,j) = pressure(i,j) + source_term
...
!--------------------------------------------------------
  ...
  do j = 2,NY
    do i = 2,NX
      ...
      value_dpressure_dx = (pressure(i,j) - pressure(i-1,j)) / DELTAX

      memory_dpressure_dx(i,j) = b_x(i) * memory_dpressure_dx(i,j) + a_x(i) * value_dpressure_dx

      value_dpressure_dx = value_dpressure_dx / K_x(i) + memory_dpressure_dx(i,j)
...
!     interpolate density at the right location in the staggered grid cell
      ...
      value_dpressure_dy = (pressure(i,j+1) - pressure(i,j)) / DELTAY
